...
namespace Web_Project.Controllers
{
    ...
    public class BooksController : Controller
    {
        ...
        public async Task<IActionResult> Index(string category = "Fantasy")
        {
            try
            {
                _logger.LogInformation($"Fetching books for category: {category}");

                var categoryObj = await _categoryRepo.GetCategoryByNameAsync(category);
                if (categoryObj == null)
                {
                    _logger.LogWarning($"Category '{category}' not found.");
                    return NotFound();
                ...
                var books = await _bookRepo.GetBooksByCategoryAsync(categoryObj.CategoryID);
